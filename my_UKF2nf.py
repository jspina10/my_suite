...
def plot_qxxx_2d(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2nf_qfrc.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_uxxx_2d(uxxx, muscle_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2nf_ctrl.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
...
output_path = os.path.join(os.path.dirname(__file__), "trajectories/simulation/kinematics_predicted_ukf2nf.csv")
