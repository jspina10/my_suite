...
from folders._myosuite.envs.myo import myobase
from folders.filterpy.kalman import UnscentedKalmanFilter as UKF
from folders.filterpy.kalman import MerweScaledSigmaPoints
...
def plot_qxxx(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKF2nf_qpos.png')  # Save the plot to a file
...
def plot_qxxx_2d(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKFnf_qfrc.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_uxxx_2d(uxxx, muscle_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKF2nf_ctrl.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
...
output_path = os.path.join(os.path.dirname(__file__), "kinematics_predicted_ukf2nf.csv")
pd.DataFrame(kinematics_predicted).to_csv(output_path, index=False, header=False)
output_path = os.path.join(os.path.dirname(__file__), "kinetics_predicted_ukf2nf.csv")
