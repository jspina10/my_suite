...
def plot_qxxx(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKF1f_qpos.png')  # Save the plot to a file
...
def plot_qxxx_2d(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKF1f_qfrc.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_uxxx_2d(uxxx, muscle_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('folders/traj_standard/graphs/UKF1f_ctrl.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
...
# env = gym.make("my_MyoFingerForce-v0", frame_skip=1, normalize_act=False)
...
# SAVE
output_name = os.path.join(os.path.dirname(__file__), "videos/ukf1f.mp4")
skvideo.io.vwrite(output_name, np.asarray(frames),outputdict={"-pix_fmt": "yuv420p"})
output_path = os.path.join(os.path.dirname(__file__), "kinematics_predicted_ukf1f.csv")
