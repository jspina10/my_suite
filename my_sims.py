...
def plot_qxxx(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_qpos_sims.png')  # Save the plot to a file
...
def plot_qxxx222(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_qpos_sims2.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_qxxx_2(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_qfrc_sims.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_qxxx_2d(qxxx, joint_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_qfrc_sims_noref.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_uxxx_2(uxxx, muscle_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_ctrl_sims.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
def plot_uxxx_2d(uxxx, muscle_names, labels):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('graphs/UKF2_ctrl_sims_noref.png')  # Save the plot to a file
    plt.close()  # Close the figure to free memory
...
print(f'error max [Reference - Predicted qpos] (rad): {error_rad.max()}')
print(f'error max [Reference - Predicted qpos] (deg): {error_deg.max()}')
error_rad = np.sqrt(((all_qpos_2[:,1:,0] - all_qpos_2[:,1:,-1])**2)).mean(axis=0)
error_deg = (180*error_rad)/np.pi
print(f'error max [Reference - DI+QP qpos] (rad): {error_rad.max()}')
print(f'error max [Reference - DI+QP qpos] (deg): {error_deg.max()}')
joint_names = [model_test.joint(i).name for i in range(model_test.nq)]
plot_qxxx(all_qpos, joint_names, ['Predicted qpos', 'Reference - Predicted qpos'])
plot_qxxx222(all_qpos_2, joint_names, ['Predicted qpos', 'Reference - DI+QP qpos'])
plot_qxxx_2(all_qfrc, joint_names, ['Predicted qfrc', 'Reference qfrc'])
plot_qxxx_2d(all_qfrc[:,:,0], joint_names, ['Predicted qfrc'])
...
plot_uxxx_2d(all_ctrl[:,:,0], muscle_names, ['Predicted ctrl'])

output_name = os.path.join(os.path.dirname(__file__), "videos/my_sims_notpredicted.mp4")
skvideo.io.vwrite(output_name, np.asarray(frames_1),outputdict={"-pix_fmt": "yuv420p"})
output_name = os.path.join(os.path.dirname(__file__), "videos/my_sims_predicted.mp4")
